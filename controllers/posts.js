...
export const updatePost = async(req,res)=>{
    ...
    const post = req.body;
    if(!mongoose.Types.ObjectId.isValid(_id))  return res.status(404).send('Not post with this id');
...
export const deletePost = async(req,res)=>{
    const{id}=req.params;
    if(!mongoose.Types.ObjectId.isValid(id))  return res.status(404).send('Not post with this id');
